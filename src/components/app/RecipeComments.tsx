...
type Comment =
  RouterOutputs["public"]["comment"]["getCommentsByRecipeId"][number];
...
export const RecipeComments = ({
  ...
  const { data: comments = initialComments } =
    api.public.comment.getCommentsByRecipeId.useQuery({
      recipeId,
      userId: session?.user?.id,
    });

  const addComment = api.authenticated.comment.addComment.useMutation({
    onSuccess: () => {
      setCommentText("");
      toast.success("Comment added successfully!");
      void utils.public.comment.getCommentsByRecipeId.invalidate({
        recipeId,
        userId: session?.user?.id,
      });
    },
  ...
  const editComment = api.authenticated.comment.editComment.useMutation({
    onSuccess: () => {
  ...
  const deleteComment = api.authenticated.comment.deleteComment.useMutation({
    onSuccess: () => {
  ...
  const likeComment = api.authenticated.comment.likeComment.useMutation({
    onSuccess: () => {
      void utils.public.comment.getCommentsByRecipeId.invalidate({
        recipeId,
        userId: session?.user?.id,
  ...
  const unlikeComment = api.authenticated.comment.unlikeComment.useMutation({
    onSuccess: () => {
      void utils.public.comment.getCommentsByRecipeId.invalidate({
        recipeId,
        userId: session?.user?.id,
            ...
            return (
              <div key={comment.id} className="rounded-lg border bg-card p-4">
                ...
                {editingCommentId === comment.id ? (
                  ...
                  <p className="wrap-break-word break-all text-sm">
                    {comment.text}
                  </p>
                )}
              </div>
